...
const StyledImage = styled.img`
  ${props => props.css}
`;
const StyledClickImage = styled.a`
  ${props => props.css}
`;
...
export const ContactButton = props => {
  ...
  return (
    <a className="bp-btn-hero bp-upper" href={contactID} onClick={contactMe}>
      contact
    </a>
  );
};
...
export const Image = props => {
  const { style, image, width, height, clickable, onclick } = props;
...
export const Heading = props => {
  ...
  const TagName = tag || 'p';
  return <TagName className={headerClasses}>{text}</TagName>;
};

export const Divider = props => {
  const { style } = props;
  return style && style.length > 1 ? (
    <StyledDiv css={style} className={props.class} />
  ) : (
...
export const Header = props => {
  ...
  return (
    <div>
      <StyledDiv css={style} id={id} className={headerClasses}>
        {title}
      </StyledDiv>
      {SubTitle}
      {Quote}
...
export class ContactForm extends React.Component {
  ...
  handleSubmit(e) {
    if (this.state.message.length < 2 || this.state.name.length < 1) {
      alert(
        `Sorry, can't send an incomplete message, Please provide all details`,
      );
      e.preventDefault();
  ...
  render() {
    return (
      <form className="bp-form contact-form" onSubmit={this.handleSubmit}>
        <TextField label="Name" className="bp-text-field">
          <Input
        ...
        <TextField label="Email" className="bp-text-field">
          <Input
            type="text"
            inputMode="email"
        ...
        <TextField label="Phone" className="bp-text-field">
          <Input
            type="text"
            inputMode="tel"
        ...
        <h1>
          <button className="bp-btn contact-btn">
            <span className="bp-upper">Submit</span>
          </button>
        </h1>
      </form>
    );
